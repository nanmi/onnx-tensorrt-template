...
//by nanmi
void Trt::CopyFromHostToDevice(float* input, int bindIndex) 
{
    mBinding[bindIndex] = static_cast<void*>(input);
}
...
// by nanmi
void Trt::CopyFromDeviceToHost(float* output, int bindIndex)
{
    output = static_cast<float*>(mBinding[bindIndex]);
    int size = mBindingSize[bindIndex]/sizeof(float);
}
